...
List<Transactions> monthType(List<Transactions> list, DateTime month) {
  List<Transactions> transac = [];

  for (int i = 0; i < list.length; i++) {
    if (list[i].dateTime.month == month.month) {
      transac.add(list[i]);
    }
  }
      ...
      child: ValueListenableBuilder(
        ...
        builder: (context, Box<Transactions> tra, _) {
          List<Transactions> boxList =
              monthType(tra.values.toList(), thisMonth);
            ...
            children: [
              Row(
                children: [
                  ...
                  Text(DateFormat('MMM-yyyy').format(thisMonth)),
                  IconButton(
                    onPressed: () {
                      setState(() {
                        thisMonth =
